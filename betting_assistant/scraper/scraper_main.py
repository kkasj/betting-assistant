...
def load_match_links(max = 200):
    ...
    # if there isn't one in our database, fetch it from the site
        ...
        pending_results.to_csv(package_path + '/data/pending_results.csv', index=False, header=False)
    
    # try to load match data dataframe for tomorrow
    try:
        df2 = pd.read_csv(package_path + '/data/match_data/' + d2 + '.csv')
    ...
    # if there isn't one in our database, fetch it from the site
        ...
        save_data(match_data, package_path + '/data/match_data/', tomorrow)
...
def fill_pending_results():
    ...
    for datestr in pending_results[:3]:
        ...
        save_data(results, package_path + '/data/results/', str2date(datestr))
    pending_results = pending_results[3:] + pending_results1 # delete fetched results from the pending list
    
    pd.Series(pending_results, dtype='object').to_csv(package_path + '/data/pending_results.csv', index=False, header=False)
